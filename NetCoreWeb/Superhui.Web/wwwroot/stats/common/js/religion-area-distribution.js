...
function drawReligionSiteProportionCharts(currentAreaInfo) {
	...
	var legendData = ["佛教", "道教", "伊斯兰教", "天主教", "基督教"];
	var seriesData = [
	...
	console.log(AreaSelectedValue.provinceCode);
	
	if (!isHeNan(AreaSelectedValue.provinceCode)) {
		legendData.push("民间信仰");
		seriesData.push({ value: currentAreaInfo.folk_rlg_site_num, name: "民间信仰" });
	}
...
function drawReligionTeacherProportionCharts(currentAreaInfo) {
	...
	if (!isHeNan(AreaSelectedValue.provinceCode)) {
		legendData.push("民间信仰");
		seriesData.push({ value: currentAreaInfo.folk_rlg_tch_num, name: "民间信仰" });
	}
	var echartsOption = {
		title: {
			text: "宗教教职人员占比情况",
...
function drawReligionTeacherDistributionCharts(childrenAreaInfoArray) {
	...
	var legendData = ["佛教", "道教", "伊斯兰教", "天主教", "基督教", "民间信仰"];
	var seriesData = [
		{
			name: '佛教',
			type: 'bar',
...
function isHeNan(code) {
	return code === "410000000000";
}
